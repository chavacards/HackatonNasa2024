...
extern void event_moveRight(GtkWidget *rightBot, gpointer data)
{
    ...
    else
    {
        ...
        sprintf(text, "/home/ict23sus/Hackaton/hackaton/nasa.jpg");
...
{   
    ...
    printf("event_mainMenu triggered\n");
    ref *nav = (ref *)data;
    nav->lblEmergentWindow = CreateWindow("ExoPlanet's Catalog", G_CALLBACK(closeWindow), nav);
    printf("gol");
    char text[250];
    int num;
    ...
    if (nav->first != NULL)
    {
        ...
        label1 = gtk_label_new("Welcome to the Exoplanet's catalog");

        // Botones de la LDC
        label2 = gtk_button_new_with_label("Giant Gas");
        label3 = gtk_button_new_with_label("Terrestrial");
        // foto
        sprintf(text, " ");
        nav->photo = gtk_image_new_from_file(text);
        label4 = gtk_button_new_with_label("Super Earth");
        label5 = gtk_button_new_with_label("Neptune");
        label6 = gtk_button_new_with_label("Unknown");

        bottonExit = gtk_button_new_with_label("Exit section");
        ...
        gtk_signal_connect(GTK_OBJECT(bottonExit), "clicked", GTK_SIGNAL_FUNC(closeWindow), nav);

        // Mover a la siguiente foto
        gtk_signal_connect(GTK_OBJECT(label2), "clicked", GTK_SIGNAL_FUNC(event_giantGas), nav);

        // Nueva foto
        gtk_signal_connect(GTK_OBJECT(label3), "clicked", GTK_SIGNAL_FUNC(event_terrestrial), nav);

        // Dar de alta
        gtk_signal_connect(GTK_OBJECT(label4), "clicked", GTK_SIGNAL_FUNC(event_superEarth), nav);

        // Dar de baja
        gtk_signal_connect(GTK_OBJECT(label5), "clicked", GTK_SIGNAL_FUNC(event_neptune), nav);

        // Borrar la última foto
        gtk_signal_connect(GTK_OBJECT(label6), "clicked", GTK_SIGNAL_FUNC(event_unknow), nav);

        // 4° Definiendo la jerarquía de instancias (pack de widgets)

        // 1° Caja vertical
        gtk_box_pack_start_defaults(GTK_BOX(hBox1), label1);
        ...
        gtk_box_pack_start_defaults(GTK_BOX(hBox2), label3);
        // Inserto la 2da caja en la caja principal
        gtk_box_pack_start_defaults(GTK_BOX(vBox), hBox2);

        // 3° Caja vertical
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->photo);
        // Inserto la 3era caja en la caja principal
        gtk_box_pack_start_defaults(GTK_BOX(vBox), hBox3);

        // 4° Caja vertical
        gtk_box_pack_start_defaults(GTK_BOX(hBox4), label4);
        gtk_box_pack_start_defaults(GTK_BOX(hBox4), label5);
        gtk_box_pack_start_defaults(GTK_BOX(hBox4), bottonExit);
...
extern void event_planetInfo(GtkWidget *lblType, gpointer data)
{
    ...
    GtkWidget *vBox, *hBox1, *hBox2, *hBox3, *hBox4;
    GtkWidget *label2, *label3, *label4, *label5, *label6, *buttonExit;
    ...
    if (nav->first != NULL)
    {
        ...
        buttonExit = gtk_button_new_with_label("Return");
        nav->leftBot = gtk_button_new_with_label(">");
        ...
        gtk_signal_connect(GTK_OBJECT(nav->lblEmergentWindow), "destroy", GTK_SIGNAL_FUNC(closeWindow), nav);
        // Cerrar programa
        gtk_signal_connect(GTK_OBJECT(buttonExit), "clicked", GTK_SIGNAL_FUNC(closeWindow), nav);
        // Mover Izquierda
        ...
        // 1° Caja vertical
        gtk_box_pack_start_defaults(GTK_BOX(hBox1), nav->lblPlanetClass);
        // Inserto la 1er caja en la caja principal
        gtk_box_pack_start_defaults(GTK_BOX(vBox), hBox1);

        // 2° Caja vertical
        gtk_box_pack_start_defaults(GTK_BOX(hBox2), nav->photo);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblNamePlanet);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblDescription);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblDistanceEarth);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblPlanetRadius);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblDiscoveryMethod);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblPlanetMass);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblDiscoveryDate);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblOrbitalRadius);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblOrbitalPeriod);
        gtk_box_pack_start_defaults(GTK_BOX(hBox3), nav->lblEccentricity);

        gtk_box_pack_start_defaults(GTK_BOX(vBox), hBox3);

        gtk_box_pack_start_defaults(GTK_BOX(vBox), hBox2);

        gtk_box_pack_start_defaults(GTK_BOX(hBox4), nav->leftBot);
        gtk_box_pack_start_defaults(GTK_BOX(hBox4), nav->buttonInsert);
        gtk_box_pack_start_defaults(GTK_BOX(hBox4), nav->buttonErase);
        gtk_box_pack_start_defaults(GTK_BOX(hBox4), nav->buttonMap);
        gtk_box_pack_start_defaults(GTK_BOX(hBox4), buttonExit);
        gtk_box_pack_start_defaults(GTK_BOX(hBox4), nav->rightBot);
        gtk_box_pack_start_defaults(GTK_BOX(vBox), hBox4);
    }
    ...
    if (nav->first != NULL)
    {
        ...
        while (currentNode != NULL)
        {
            if (strcmp(currentNode->dataExoP.planetClass, "Giant Gas") == 0)
            {
                ...
                sprintf(text, "/home/ict23sus/Hackaton/hackaton/%s.jpg", currentNode->dataExoP.name);
...
void closeWindow(GtkButton *button, gpointer data)
{
    // Typecast para tener referencia de info
    ref *base = (ref *)data;
    int cont;
    char text[100];

    gtk_window_set_modal(GTK_WINDOW(base->lblEmergentWindow), FALSE);
    gtk_widget_destroy(GTK_WIDGET(base->lblEmergentWindow));
}
/**
 *
 */
